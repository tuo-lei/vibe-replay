...
function stripTerminalTranscriptNoise(text: string): string {
  let cleaned = text;
  cleaned = cleaned.replace(/^Last login:[^\n]*(?:\n|$)/gim, "");
  cleaned = cleaned.replace(/^\s*(?:➜|❯)\s+[^\n]*(?:\n|$)/gm, "");
  cleaned = cleaned.replace(
    /^\s*[-*]\s+(?:Active account|Git operations protocol):[^\n]*(?:\n|$)/gim,
    "",
  );
  cleaned = cleaned.replace(/^\s*[✓✔]\s+Logged in to github\.com[^\n]*(?:\n|$)/gim, "");
  return cleaned;
}
...
export function cleanPrompt(text: string): string {
  ...
  let cleaned = stripTerminalTranscriptNoise(text);
  ...
  if (hadTerminalNoise && /^[a-z0-9._:-]{1,8}$/i.test(cleaned)) return "";
  return cleaned;
}
...
export function sourceDisplayTitle(
  ...
  if (replayTitle && replayTitle !== promptTitle) return replayTitle;
