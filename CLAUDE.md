...
- **Replay compaction snapshots**: Generated replay cards must use the replay summary's own `compactionCount`, including an explicit `0`. Only legacy summaries where the field is absent may fall back to the current source scan; otherwise a replay generated before a later source compaction is mislabeled.
