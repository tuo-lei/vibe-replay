...
- **Grok Bot**: Cloud-box JSONL at `/home/box/agent-data/agent-transcripts` (and `sand-data`); env `GROK_BOT_TRANSCRIPTS_DIR` / `VIBE_REPLAY_GROK_BOT_DIR`; optional `~/.grok-bot/agent-transcripts`. Skip `[SAND_HIDDEN_PROMPT]`, strip `[t0u]` prefixes, promote `send_message` to assistant text. Details: `packages/provider-grok-bot/AGENTS.md`.
